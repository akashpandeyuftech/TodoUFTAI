...
import { getTeamTodos, toggleTodo, deleteTodo, createTodo, claimTodo, releaseTodo } from "@/lib/api";
...
export default function TeamTodosScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setTodos(await getTeamTodos(filter));
    } catch {
      Alert.alert("Failed to load team tasks");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  async function handleRelease(id: string) {
    try {
      await releaseTodo(id);
      await load();
    } catch (e: unknown) {
      Alert.alert("Error", e instanceof Error ? e.message : "Failed to release");
    }
  }

  async function handleCreate(payload: CreateTodoPayload) {
    await createTodo({ ...payload, owner_type: "team" });
    await load();
  }
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>No team tasks yet.</Text>}
        />
      )}
